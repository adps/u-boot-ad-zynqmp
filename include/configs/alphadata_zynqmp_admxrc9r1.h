...
#ifndef __CONFIG_ZYNQMP_ADMVPX39Z_H
#define __CONFIG_ZYNQMP_ADMVPX39Z_H
...
#define CONFIG_EXTRA_ENV_BOARD_SETTINGS \
	...
	"uenvboot=" \
		...
		"if test -n $uenvcmd; then " \
			...
			"run uenvcmd; " \
		"fi\0" \
	"sdboot=mmc dev 0 && mmcinfo && run uenvboot;"\
		"     load mmc 0 $fdt_addr $fdt_image && load mmc 0 $kernel_addr $kernel_image && load mmc 0 $ramdisk_addr $ramdisk_image && booti $kernel_addr $ramdisk_addr $fdt_addr\0" \
...
#include <configs/alphadata_zynqmp.h>

#endif /* __CONFIG_ZYNQMP_ADMVPX39Z_H */
